function updateBadge() {
    ...
    if (el && window.Cart) el.textContent = Cart.cartCount();
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!id) {
        box.innerHTML = `<div class="alert alert-danger">URLda id yo'q. Masalan: product.html?id=1</div>`;
        return;
    }

    if (!window.PRODUCTS || !Array.isArray(window.PRODUCTS)) {
        box.innerHTML = `<div class="alert alert-danger">PRODUCTS topilmadi. data.js yuklanmagan (yo'l yoki tartib xato).</div>`;
        return;
    }

    const p = (window.getProductById ? getProductById(id) : window.PRODUCTS.find(x => x.id === Number(id)));

    if (!p) {
        box.innerHTML = `
      <div class="alert alert-danger">
        Mahsulot topilmadi. ID: <b>${id}</b><br>
        PRODUCTS uzunligi: <b>${window.PRODUCTS.length}</b>
      </div>`;
        return;
    }
